...
export class ButtonForUpdateGoodsComponent extends Vue {
  @Prop()
  record: any;
  ...
  onConfirm() {
    this.$refs.form.form.validateFields(async (errors, values) => {
      ...
      await goodsService.update(this.record.id, {
        ...rest,
...
export interface ButtonForUpdateGoodsComponent {
  $refs: {
    form: EditingFormComponent;
  };
}

export default ButtonForUpdateGoodsComponent;
